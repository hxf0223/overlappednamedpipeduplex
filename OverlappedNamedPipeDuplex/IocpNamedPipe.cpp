...
namespace boost
{
	namespace interprocess
	{
		namespace overlapped
		{
			...
			{
			public:
				explicit COverlappedNamedPipeDuplexImp(std::string appName, OVNamedPipeDuplexServerCb cbServer, OVNamedPipeDuplexClientCb cbClient);
				virtual ~COverlappedNamedPipeDuplexImp();
			...
			private:
				...
				static void pipe_thread_client(HANDLE& hEvent, std::string& strAppName, OVNamedPipeDuplexClientCb serverCb);
				static void pipe_thread_server(HANDLE& hEvent, std::vector<PipeInstance>* pInstArray, OVNamedPipeDuplexServerCb serverCb);
				...
				static void disconnect_pipes(std::vector<HANDLE>* hs);
				static void close_events(std::vector<HANDLE>* hs);
			...
			enum class COverlappedNamedPipeDuplexImp::PipeState : byte
			{
			...
			struct COverlappedNamedPipeDuplexImp::PipeInstance
			{
				OVERLAPPED overlap;
				HANDLE hPipeInst;
			...
			const std::string COverlappedNamedPipeDuplexImp::sm_pipe_name = get_pipe_prefix() + windowify_pipe_path("udsAppMutex");
			const int COverlappedNamedPipeDuplexImp::sm_pipe_create_buffer_size_input = 1024;
			const int COverlappedNamedPipeDuplexImp::sm_pipe_create_buffer_size_output = 1024;
			const int COverlappedNamedPipeDuplexImp::sm_pipe_ins_num = 5;

			/**
			 * \brief 
			 * \param appName 
			 */
			COverlappedNamedPipeDuplexImp::COverlappedNamedPipeDuplexImp(std::string appName, OVNamedPipeDuplexServerCb cbServer, OVNamedPipeDuplexClientCb cbClient) :
				m_appname(appName),
			...
			COverlappedNamedPipeDuplexImp::~COverlappedNamedPipeDuplexImp() {
				stopServer();
				stopClient();
			}

			std::string COverlappedNamedPipeDuplexImp::get_pipe_name() {
				return sm_pipe_name;
			}

			void COverlappedNamedPipeDuplexImp::startServer() {
				if ( m_thread_server ) {
				...
				m_thread_server = std::make_unique<boost::thread>(COverlappedNamedPipeDuplexImp::pipe_thread_server, 
					COverlappedNamedPipeDuplexImp::m_hServerExitEvent.get(), COverlappedNamedPipeDuplexImp::m_pis.get(), 
					COverlappedNamedPipeDuplexImp::m_servercb);
			}

			void COverlappedNamedPipeDuplexImp::stopServer() {
				if ( m_thread_server ) {
					SetEvent(m_hServerExitEvent.get());
					m_thread_server->join();
					m_thread_server.reset();
			...
			void COverlappedNamedPipeDuplexImp::startClient() {
				if ( m_thread_client ) {
					SetEvent(m_hClientExitEvent.get());
					m_thread_client.get()->join();
					m_thread_client.reset();
				}

				auto bsuccess = ResetEvent(m_hClientExitEvent.get());
				m_thread_client = std::make_unique<boost::thread>(COverlappedNamedPipeDuplexImp::pipe_thread_client, 
					COverlappedNamedPipeDuplexImp::m_hClientExitEvent.get(), COverlappedNamedPipeDuplexImp::m_appname,
					COverlappedNamedPipeDuplexImp::m_clientcb);
			}

			void COverlappedNamedPipeDuplexImp::stopClient() {
				if ( m_thread_client ) {
					SetEvent(m_hClientExitEvent.get());
			...
			void COverlappedNamedPipeDuplexImp::pipe_thread_client(HANDLE& hEvent, std::string& strAppName, OVNamedPipeDuplexClientCb clientCb) {
				...
				auto dwWait = WaitForSingleObject(hEvent, 0);
				...
				while ( dwWait != WAIT_OBJECT_0 ) {
					...
					if ( !WriteFile(hPipe, &request_data, cbToWrite, &cbWritten, nullptr) ) {
			...
			void COverlappedNamedPipeDuplexImp::pipe_thread_server(HANDLE& hEvent, std::vector<PipeInstance>* pInstArray, OVNamedPipeDuplexServerCb serverCb) {
				...
				while ( true ) {
					auto dwWait = WaitForMultipleObjects(pipe_ins_size + 1, &(hall_events[0]), FALSE, INFINITE);
					auto waitid = dwWait - WAIT_OBJECT_0;
			...
			void COverlappedNamedPipeDuplexImp::disconnect_pipe(HANDLE handle) {
				PipeScopedHandle psh(handle);
			}

			void COverlappedNamedPipeDuplexImp::disconnect_pipes(std::vector<HANDLE>* hs) {
				bforeach(auto& x, *hs) {
			...
			void COverlappedNamedPipeDuplexImp::close_events(std::vector<HANDLE>* hs) {
				bforeach(auto& x, *hs) {
			...
			void COverlappedNamedPipeDuplexImp::close_event(HANDLE h) {
				EventScopedHandle eh(h);
			}

			boost::tuple<BOOL, COverlappedNamedPipeDuplexImp::PipeState> COverlappedNamedPipeDuplexImp::disconn_and_reconn_pipe(HANDLE hPipeInst, LPOVERLAPPED lpo) {
			...
			BOOL COverlappedNamedPipeDuplexImp::conn_to_new_client(HANDLE hPipe, LPOVERLAPPED lpo) {
				auto fConnected = ConnectNamedPipe(hPipe, lpo);
			...
			COverlappedNamedPipeDuplex::COverlappedNamedPipeDuplex(std::string appName, OVNamedPipeDuplexServerCb cbServer, OVNamedPipeDuplexClientCb cbClient) :
				m_pipe_comm_imp(new COverlappedNamedPipeDuplexImp(appName, cbServer, cbClient)) {
			}

			COverlappedNamedPipeDuplex::~COverlappedNamedPipeDuplex() = default;

			std::string COverlappedNamedPipeDuplex::get_pipe_name() {
				return COverlappedNamedPipeDuplexImp::get_pipe_name();
			}

			void COverlappedNamedPipeDuplex::startServer() const {
				m_pipe_comm_imp->startServer();
			}

			void COverlappedNamedPipeDuplex::stopServer() const {
				m_pipe_comm_imp->stopServer();
			}

			void COverlappedNamedPipeDuplex::startClient() const {
				m_pipe_comm_imp->startClient();
			}

			void COverlappedNamedPipeDuplex::stopClient() const {
